...
object LookupApplication {
  ...
  def startRemoteCalculatorSystem(): Unit = {
    val system = ActorSystem("CalculatorSystem",ConfigFactory.load("remote/calc/calculator"))
  ...
  def startRemoteLookupSystem(): Unit = {
    val system =ActorSystem("LookupSystem", ConfigFactory.load("remote/calc/remoteLookup"))
