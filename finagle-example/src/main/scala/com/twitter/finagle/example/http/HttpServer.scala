 ...
 */
object HttpServer {
  val log = LoggerFactory.getLogger("com.twitter.finagle.example.http.HttpServer")
  /**
  ...
  class Authorize extends SimpleFilter[Request, Response] {
    def apply(request: Request, continue: Service[Request, Response]) = {
      log.debug("headerMap:{}",request.headerMap)
      if (request.headerMap.get(Fields.Authorization).contains("openSesame")) {
        continue(request)
      } else {
